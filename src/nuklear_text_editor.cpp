...
intERN void nk_textedit_makeundo_delete(struct nk_text_edit*, int, int);
intERN void nk_textedit_makeundo_insert(struct nk_text_edit*, int, int);
intERN void nk_textedit_makeundo_replace(struct nk_text_edit*, int, int, int);
    ...
    if (state->undo_point > 0) {
        ...
        if (state->undo_rec[0].char_storage >= 0) {
            ...
            std::memcpy(state->undo_char.data(), state->undo_char.data() + n,
                (std::size_t)state->undo_char_point*sizeof(nk_rune));
...
intERN struct nk_text_undo_record*
...
NK_API void
nk_textedit_init_fixed(struct nk_text_edit *state, void *memory, std::size_t size)
{
    NK_ASSERT(state);
    NK_ASSERT(memory);
    if (!state || !memory || !size) return;
    std::memset(state, 0, sizeof(struct nk_text_edit));
    nk_textedit_clear_state(state, NK_TEXT_EDIT_SINGLE_LINE, 0);
    nk_str_init_fixed(&state->string, memory, size);
}
NK_API void
nk_textedit_init(struct nk_text_edit *state, const struct nk_allocator *alloc, std::size_t size)
{
    NK_ASSERT(state);
    NK_ASSERT(alloc);
