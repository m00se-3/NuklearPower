 ...
 *
 * ===============================================================*/
intERN int nk_str_match_here(const char *regexp, const char *text);
intERN int nk_str_match_star(int c, const char *regexp, const char *text);
NK_LIB bool nk_is_lower(int c) {return (c >= 'a' && c <= 'z') || (c >= 0xE0 && c <= 0xFF);}
NK_LIB bool nk_is_upper(int c){return (c >= 'A' && c <= 'Z') || (c >= 0xC0 && c <= 0xDF);}
NK_LIB int nk_to_upper(int c) {return (c >= 'a' && c <= 'z') ? (c - ('a' - 'A')) : c;}
NK_LIB int nk_to_lower(int c) {return (c >= 'A' && c <= 'Z') ? (c - ('a' + 'A')) : c;}

NK_LIB void
nk_zero(void *ptr, std::size_t size)
{
    NK_ASSERT(ptr);
...
nk_murmur_hash(const void * key, int len, nk_hash seed)
{
    ...
    k1 = 0;
    switch (len & 3) {
        case 3: k1 ^= tail[2] << 16u; /* fallthrough */
        case 2: k1 ^= tail[1] << 8u; /* fallthrough */
        case 1: k1 ^= tail[0];
            k1 *= c1;
            k1 = NK_ROTL(k1,15);
            k1 *= c2;
...
#ifdef NK_INCLUDE_STANDARD_IO
NK_LIB char*
nk_file_load(const char* path, std::size_t* siz, const struct nk_allocator *alloc)
{
    char *buf;
    FILE *fd;
    long ret;
