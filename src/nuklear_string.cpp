...
namespace nk {
  ...
  str_append_text_char(str* s, const char* str, const int len) {
    ...
    if (!s || !str || !len)
      return 0;
    char* mem = (char*) buffer_alloc(&s->buffer, buffer_allocation_type::BUFFER_FRONT, static_cast<std::size_t>(len) * sizeof(char), 0);
    if (!mem)
      return 0;
  ...
  str_append_str_runes(str* str, const rune* runes) {
    ...
    if (!str || !runes)
      return 0;
    while (runes[i] != '\0') {
      const int byte_len = utf_encode(runes[i], glyph, NK_UTF_SIZE);
      str_append_text_char(str, glyph, byte_len);
      i++;
    }
    return i;
  ...
  str_insert_at_char(str* s, const int pos, const char* str, const int len) {
    ...
    const int copylen = (int) s->buffer.allocated - pos;
    if (!copylen) {
      str_append_text_char(s, str, len);
      return 1;
    }
    void* mem = buffer_alloc(&s->buffer, buffer_allocation_type::BUFFER_FRONT, (std::size_t) len * sizeof(char), 0);
    if (!mem)
      return 0;
    ...
    char* dst = ptr_add(char, s->buffer.memory.ptr, pos + len + (copylen - 1));
    const char* src = ptr_add(char, s->buffer.memory.ptr, pos + (copylen - 1));
  ...
  str_insert_at_rune(str* str, const int pos, const char* cstr, const int len) {
    ...
    if (!str || !cstr || !len)
      return 0;
    const char* begin = str_at_rune(str, pos, &unicode, &glyph_len);
    if (!str->len)
      return str_append_text_char(str, cstr, len);
    const char* buffer = str_get_const(str);
    if (!begin)
      return 0;
  ...
  str_remove_runes(str* str, int len) {
    ...
    const int index = str->len - len;
    const char* begin = str_at_rune(str, index, &unicode, &len);
  ...
  str_delete_chars(str* s, const int pos, const int len) {
    ...
    if ((std::size_t) (pos + len) < s->buffer.allocated) {
      ...
      const char* src = ptr_add(char, s->buffer.memory.ptr, pos + len);
  ...
  str_delete_runes(str* s, const int pos, int len) {
    ...
    char* temp = (char*) s->buffer.memory.ptr;
    char* begin = str_at_rune(s, pos, &unicode, &unused);
    if (!begin)
      return;
    s->buffer.memory.ptr = begin;
    const char* end = str_at_rune(s, len, &unicode, &unused);
  ...
  str_at_rune(str* str, const int pos, rune* unicode, int* len) {
    ...
    char* text = (char*) str->buffer.memory.ptr;
    const int text_len = (int) str->buffer.allocated;
  ...
  str_at_const(const str* str, const int pos, rune* unicode, int* len) {
    ...
    const char* text = (char*) str->buffer.memory.ptr;
    const int text_len = (int) str->buffer.allocated;
    glyph_len = utf_decode(text, unicode, text_len);
    while (glyph_len) {
      if (i == pos) {
