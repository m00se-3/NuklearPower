...
NK_API void
nk_str_init(struct nk_str *str, const struct nk_allocator *alloc, std::size_t size)
{
    nk_buffer_init(&str->buffer, alloc, size);
    str->len = 0;
}
NK_API void
nk_str_init_fixed(struct nk_str *str, void *memory, std::size_t size)
{
...
nk_str_append_text_char(struct nk_str *s, const char *str, int len)
{
    ...
    mem = (char*)nk_buffer_alloc(&s->buffer, NK_BUFFER_FRONT, (std::size_t)len * sizeof(char), 0);
    if (!mem) return 0;
...
nk_str_insert_at_char(struct nk_str *s, int pos, const char *str, int len)
{
    ...
    NK_ASSERT(str);
    NK_ASSERT(len >= 0);
    if (!s || !str || !len || (std::size_t)pos > s->buffer.allocated) return 0;
    ...
    mem = nk_buffer_alloc(&s->buffer, NK_BUFFER_FRONT, (std::size_t)len * sizeof(char), 0);
    if (!mem) return 0;
...
nk_str_remove_chars(struct nk_str *s, int len)
{
    NK_ASSERT(s);
    NK_ASSERT(len >= 0);
    if (!s || len < 0 || (std::size_t)len > s->buffer.allocated) return;
...
nk_str_delete_chars(struct nk_str *s, int pos, int len)
{
    ...
    if (!s || !len || (std::size_t)pos > s->buffer.allocated ||
        (std::size_t)(pos + len) > s->buffer.allocated) return;
