...
nk_rgb_hex(const char *rgb)
{
    ...
    const char *c = rgb;
    if (*c == '#') c++;
    col.r = (std::uint8_t)nk_parse_hex(c, 2);
    col.g = (std::uint8_t)nk_parse_hex(c+2, 2);
    col.b = (std::uint8_t)nk_parse_hex(c+4, 2);
    col.a = 255;
    return col;
}
NK_API struct nk_color
nk_rgba_hex(const char *rgb)
{
    ...
    col.g = (std::uint8_t)nk_parse_hex(c+2, 2);
    col.b = (std::uint8_t)nk_parse_hex(c+4, 2);
    col.a = (std::uint8_t)nk_parse_hex(c+6, 2);
    return col;
}
NK_API void
...
NK_API struct nk_color
nk_rgba_bv(const std::uint8_t *c)
{
    return nk_rgba(c[0], c[1], c[2], c[3]);
}
NK_API struct nk_color
...
NK_API struct nk_color
nk_rgb_bv(const std::uint8_t* c)
{
    return nk_rgb(c[0], c[1], c[2]);
}
NK_API struct nk_color
nk_rgba_u32(unsigned int in)
{
    struct nk_color ret;
    ret.r = (in & 0xFF);
    ret.g = ((in >> 8) & 0xFF);
...
NK_API struct nk_color
nk_hsva_bv(const std::uint8_t *c)
{
    return nk_hsva(c[0], c[1], c[2], c[3]);
}
...
NK_API void
nk_color_hsva_bv(std::uint8_t *out, struct nk_color in)
{
    int tmp[4];
...
NK_API void
nk_color_hsva_b(std::uint8_t *h, std::uint8_t *s, std::uint8_t *v, std::uint8_t *a, struct nk_color in)
{
    int tmp[4];
    nk_color_hsva_i(&tmp[0], &tmp[1], &tmp[2], &tmp[3], in);
    *h = (std::uint8_t)tmp[0];
...
NK_API void
nk_color_hsv_b(std::uint8_t *out_h, std::uint8_t *out_s, std::uint8_t *out_v, struct nk_color in)
{
    int tmp[4];
    nk_color_hsva_i(&tmp[0], &tmp[1], &tmp[2], &tmp[3], in);
    *out_h = (std::uint8_t)tmp[0];
    *out_s = (std::uint8_t)tmp[1];
    *out_v = (std::uint8_t)tmp[2];
}
NK_API void
...
NK_API void
nk_color_hsv_bv(std::uint8_t *out, struct nk_color in)
{
    int tmp[4];
