...
namespace nk {
   ...
   * ===============================================================*/
  INTERN int
  parse_hex(const char *p, int length)
  {
    int i = 0;
    int len = 0;
    while (len < length) {
  ...
  NK_API struct color
  rgb_factor(struct color col, float factor)
  {
    if (factor == 1.0f)
      return col;
  ...
  NK_API void
  color_hex_rgba(char *output, struct color col)
  {
...
#undef NK_TO_HEX
  ...
  NK_API void
  color_hex_rgb(char *output, struct color col)
  {
#define NK_TO_HEX(i) ((i) <= 9 ? '0' + (i): 'A' - 10 + (i))
    output[0] = (char)NK_TO_HEX((col.r & 0xF0) >> 4);
    output[1] = (char)NK_TO_HEX((col.r & 0x0F));
...
#undef NK_TO_HEX
  ...
  {
    struct colorf c = hsva_colorf(h, s, v, a);
    return rgba_f(c.r, c.g, c.b, c.a);
  }
  NK_API struct color
  ...
  NK_API void
  color_f(float *r, float *g, float *b, float *a, struct color in)
  {
  ...
  NK_API void
  color_fv(float *c, struct color in)
  {
    color_f(&c[0], &c[1], &c[2], &c[3], in);
  }
  NK_API struct colorf
  color_cf(struct color in)
  {
    struct colorf o;
  ...
  NK_API void
  color_d(double *r, double *g, double *b, double *a, struct color in)
  {
  ...
  NK_API void
  color_dv(double *c, struct color in)
  {
    color_d(&c[0], &c[1], &c[2], &c[3], in);
  }
  NK_API void
  color_hsv_f(float *out_h, float *out_s, float *out_v, struct color in)
  {
    float a;
    color_hsva_f(out_h, out_s, out_v, &a, in);
  }
  NK_API void
  color_hsv_fv(float *out, struct color in)
  {
    float a;
  ...
  colorf_hsva_f(float *out_h, float *out_s,
      float *out_v, float *out_a, struct colorf in)
  {
    float chroma;
    float K = 0.0f;
    if (in.g < in.b) {
  ...
  NK_API void
  colorf_hsva_fv(float *hsva, struct colorf in)
  {
    colorf_hsva_f(&hsva[0], &hsva[1], &hsva[2], &hsva[3], in);
  }
  NK_API void
  color_hsva_f(float *out_h, float *out_s,
      float *out_v, float *out_a, struct color in)
  {
    struct colorf col;
  ...
  NK_API void
  color_hsva_iv(int *out, struct color in)
  {
    color_hsva_i(&out[0], &out[1], &out[2], &out[3], in);
  }
  NK_API void
  color_hsva_bv(std::uint8_t *out, struct color in)
  {
    int tmp[4];
  ...
  NK_API void
  color_hsv_i(int *out_h, int *out_s, int *out_v, struct color in)
  {
    int a;
    color_hsva_i(out_h, out_s, out_v, &a, in);
  }
  NK_API void
  color_hsv_b(std::uint8_t *out_h, std::uint8_t *out_s, std::uint8_t *out_v, struct color in)
  {
    int tmp[4];
    color_hsva_i(&tmp[0], &tmp[1], &tmp[2], &tmp[3], in);
  ...
  NK_API void
  color_hsv_iv(int *out, struct color in)
  {
    color_hsv_i(&out[0], &out[1], &out[2], in);
  }
  NK_API void
  color_hsv_bv(std::uint8_t *out, struct color in)
  {
    int tmp[4];
