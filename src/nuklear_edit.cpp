...
nk_edit_draw_text(struct nk_command_buffer *out,
    ...
    struct nk_color foreground, bool is_selected)
{
    NK_ASSERT(out);
    NK_ASSERT(font);
    NK_ASSERT(style);
    ...
    struct nk_input *in, const struct nk_user_font *font)
{
    ...
    if (edit->active && in)
    {
        ...
        if (select_all) {
            ...
            nk_textedit_key(edit, NK_KEY_TEXT_WORD_LEFT, false, font, row_height);
            nk_textedit_key(edit, NK_KEY_TEXT_WORD_RIGHT, true, font, row_height);
            cursor_follow = true;
        }
    ...
    char *memory, int *len, int max, nk_plugin_filter filter)
{
    ...
    *len = NK_MIN(*len, max-1);
    nk_str_init_fixed(&edit->string, memory, (std::size_t)max);
