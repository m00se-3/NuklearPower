...
nk_command_buffer_push(struct nk_command_buffer* b,
    enum nk_command_type t, std::size_t size)
{
    NK_STORAGE const std::size_t align = NK_ALIGNOF(struct nk_command);
...
#define IMG_RGN(x, y, w, h) img.region[0] = (unsigned short)(x); img.region[1] = (unsigned short)(y); img.region[2] = (unsigned short)(w); img.region[3] = (unsigned short)(h);
    ...
    struct nk_color bg, struct nk_color fg)
{
    ...
    cmd = (struct nk_command_text*)
        nk_command_buffer_push(b, NK_COMMAND_TEXT, sizeof(*cmd) + (std::size_t)(length + 1));
    if (!cmd) return;
    cmd->x = (short)r.x;
    cmd->y = (short)r.y;
