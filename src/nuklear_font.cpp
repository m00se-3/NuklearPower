...
NK_GLOBAL const std::size_t nk_rect_align = NK_ALIGNOF(struct stbrp_rect);
NK_GLOBAL const std::size_t nk_range_align = NK_ALIGNOF(stbtt_pack_range);
NK_GLOBAL const std::size_t nk_char_align = NK_ALIGNOF(stbtt_packedchar);
NK_GLOBAL const std::size_t nk_build_align = NK_ALIGNOF(struct nk_font_bake_data);
NK_GLOBAL const std::size_t nk_baker_align = NK_ALIGNOF(struct nk_font_baker);
...
intERN void
nk_font_baker_memory(std::size_t *temp, int *glyph_count,
...
nk_font_bake_pack(struct nk_font_baker *baker,
    std::size_t *image_memory, int *width, int *height, struct nk_recti *custom,
    ...
    const struct nk_allocator *alloc)
{
    ...
    *height = (int)nk_round_up_pow2((unsigned int)*height);
    ...
    const struct nk_font_config *config_list, int font_count)
{
    ...
    nk_zero(image_memory, (std::size_t)((std::size_t)width * (std::size_t)height));
...
nk_font_atlas_add_from_memory(struct nk_font_atlas *atlas, void *memory,
    std::size_t size, float height, const struct nk_font_config *config)
{
    struct nk_font_config cfg;
    NK_ASSERT(memory);
    NK_ASSERT(size);
    ...
    enum nk_font_atlas_format fmt)
{
    ...
    atlas->glyphs = (struct nk_font_glyph*)atlas->permanent.alloc(
        atlas->permanent.userdata,0, sizeof(struct nk_font_glyph)*(std::size_t)atlas->glyph_count);
