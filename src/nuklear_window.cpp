...
nk_find_window(const struct nk_context *ctx, nk_hash hash, const char *name)
{
    ...
    while (iter != nullptr) {
        ...
        if (iter->name == hash) {
            ...
            if (nk_stricmpn(iter->name_string, name, max_len) == 0)
                return iter;
        }
        iter = iter->next;
    }
    ...
    struct nk_rect bounds, nk_flags flags)
{
    ...
    if (!(win->flags & NK_WINDOW_HIDDEN) && !(win->flags & NK_WINDOW_NO_INPUT))
    {
        ...
        inpanel = nk_input_has_mouse_click_down_in_rect(&ctx->input, NK_BUTTON_LEFT, win_bounds, true);
    ...
    win->layout->offset_x = reinterpret_cast<std::uint32_t*>(&win->scrollbar.x);
    win->layout->offset_y = reinterpret_cast<std::uint32_t*>(&win->scrollbar.y);
    return ret;
}
NK_API void
...
NK_API void
nk_window_get_scroll(const struct nk_context *ctx, std::uint32_t *offset_x, std::uint32_t *offset_y)
{
    struct nk_window *win;
    NK_ASSERT(ctx);
    NK_ASSERT(ctx->current);
...
NK_API void
nk_window_set_scroll(struct nk_context *ctx, std::uint32_t offset_x, std::uint32_t offset_y)
{
    struct nk_window *win;
    NK_ASSERT(ctx);
    NK_ASSERT(ctx->current);
...
NK_API void
nk_rule_horizontal(struct nk_context *ctx, struct nk_color color, bool rounding)
{
