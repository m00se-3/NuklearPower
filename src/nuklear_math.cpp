...
#define NK_ATAN2 atan2
      ...
      float pad_x, float pad_y, enum heading direction)
  {
    ...
    if (direction == heading::UP) {
      ...
      result[1] = vec2_from_floats(r.x + r.w, r.y + r.h);
      result[2] = vec2_from_floats(r.x, r.y + r.h);
    } else if (direction == heading::RIGHT) {
      result[0] = vec2_from_floats(r.x, r.y);
      result[1] = vec2_from_floats(r.x + r.w, r.y + h_half);
      result[2] = vec2_from_floats(r.x, r.y + r.h);
    } else if (direction == heading::DOWN) {
      result[0] = vec2_from_floats(r.x, r.y);
      result[1] = vec2_from_floats(r.x + r.w, r.y);
      result[2] = vec2_from_floats(r.x + w_half, r.y + r.h);
    } else {
      result[0] = vec2_from_floats(r.x, r.y + h_half);
      result[1] = vec2_from_floats(r.x + r.w, r.y);
      result[2] = vec2_from_floats(r.x + r.w, r.y + r.h);
    }
  }
}
