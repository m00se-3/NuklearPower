...
NK_API bool nk_menu_begin_label(struct nk_context *ctx,
    ...
    nk_flags align, enum nk_symbol_type sym, struct nk_vec2 size)
{
    ...
    if (nk_do_button_text_symbol(&ctx->last_widget_state, &win->buffer,
        ...
        ctx->style.font, in)) is_clicked = true;
    return nk_menu_begin(ctx, win, title, is_clicked, header, size);
}
NK_API bool
...
NK_API bool nk_menu_item_symbol_text(struct nk_context *ctx, enum nk_symbol_type sym,
    const char *text, int len, nk_flags align)
{
    return nk_contextual_item_symbol_text(ctx, sym, text, len, align);
}
NK_API bool nk_menu_item_symbol_label(struct nk_context *ctx, enum nk_symbol_type sym,
    const char *label, nk_flags align)
{
