 ...
 *
 * ===============================================================*/
NK_GLOBAL const std::uint8_t nk_utfbyte[NK_UTF_SIZE+1] = {0x80, 0, 0xC0, 0xE0, 0xF0};
NK_GLOBAL const std::uint8_t nk_utfmask[NK_UTF_SIZE+1] = {0xC0, 0x80, 0xE0, 0xF0, 0xF8};
NK_GLOBAL const std::uint32_t nk_utfmin[NK_UTF_SIZE+1] = {0, 0, 0x80, 0x800, 0x10000};
NK_GLOBAL const std::uint32_t nk_utfmax[NK_UTF_SIZE+1] = {0x10FFFF, 0x7F, 0x7FF, 0xFFFF, 0x10FFFF};
...
nk_utf_decode_byte(char c, int *i)
{
    ...
    for(*i = 0; *i < (int)NK_LEN(nk_utfmask); ++(*i)) {
        if (((std::uint8_t)c & nk_utfmask[*i]) == nk_utfbyte[*i])
            return (std::uint8_t)(c & ~nk_utfmask[*i]);
    }
    return 0;
}
NK_API int
...
{
    return (char)((nk_utfbyte[i]) | ((std::uint8_t)u & ~nk_utfmask[i]));
}
NK_API int
