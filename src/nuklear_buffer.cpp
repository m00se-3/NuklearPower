...
NK_LIB void*
nk_malloc(nk_handle unused, void *old,std::size_t size)
{
    NK_UNUSED(unused);
    NK_UNUSED(old);
    return malloc(size);
...
NK_API void
nk_buffer_init_fixed(struct nk_buffer *b, void *m, std::size_t size)
{
    NK_ASSERT(b);
    NK_ASSERT(m);
    NK_ASSERT(size);
...
NK_LIB void*
nk_buffer_realloc(struct nk_buffer *b, std::size_t capacity, std::size_t *size)
{
    void *temp;
    ...
    std::size_t size, std::size_t align)
{
    ...
    if (full) {
        ...
        capacity = NK_MAX(capacity, nk_round_up_pow2((unsigned int)(b->allocated + size)));
...
nk_buffer_mark(struct nk_buffer *buffer, enum nk_buffer_allocation_type type)
{
    NK_ASSERT(buffer);
    if (!buffer) return;
    buffer->marker[type].active = true;
...
nk_buffer_reset(struct nk_buffer *buffer, enum nk_buffer_allocation_type type)
{
    ...
    if (type == NK_BUFFER_BACK) {
        ...
        buffer->marker[type].active = false;
    } else {
