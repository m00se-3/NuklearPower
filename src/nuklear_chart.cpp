...
namespace nk {
    ...
    if (g->slots[slot].index == 0) {
      ...
      if (!(layout->flags & static_cast<decltype(layout->flags)>(window_flags::WINDOW_ROM)) && i &&
        ...
        ret |= (i->mouse.buttons[NK_BUTTON_LEFT].down &&
            i->mouse.buttons[NK_BUTTON_LEFT].clicked) ? std::to_underlying(chart_event::CHART_CLICKED) : 0;
      ...
      struct chart *chart, float value, int slot)
  {
    ...
    if (!(layout->flags & static_cast<decltype(layout->flags)>(window_flags::WINDOW_ROM)) && in &&
      ...
      ret |= (!in->mouse.buttons[NK_BUTTON_LEFT].down &&
              in->mouse.buttons[NK_BUTTON_LEFT].clicked) ? std::to_underlying(chart_event::CHART_CLICKED)
                 : 0;
  ...
  chart_push_slot(struct context *ctx, float value, int slot)
  {
    ...
    if (!ctx || !ctx->current || slot >= NK_CHART_MAX_SLOT) return false;
    if (slot >= ctx->current->layout->chart.slot) return false;

    win = ctx->current;
    if (win->layout->chart.slot < slot) return false;
    switch (win->layout->chart.slots[slot].type) {
      case chart_type::CHART_LINES:
        flags = chart_push_line(ctx, win, &win->layout->chart, value, slot); break;
      case chart_type::CHART_COLUMN:
        flags = chart_push_column(ctx, win, &win->layout->chart, value, slot); break;
      default:
