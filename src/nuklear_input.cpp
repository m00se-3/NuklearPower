...
NK_API void
nk_input_key(struct nk_context *ctx, enum nk_keys key, bool down)
{
    struct nk_input *in;
    NK_ASSERT(ctx);
    if (!ctx) return;
...
NK_API void
nk_input_button(struct nk_context *ctx, enum nk_buttons id, int x, int y, bool down)
{
    ...
    struct nk_rect b)
{
    ...
    if (!i) return false;
    btn = &i->mouse.buttons[id];
    return (nk_input_has_mouse_click_down_in_rect(i, id, b, false) &&
...
{
    if (!i) return false;
    if (!nk_input_is_mouse_hovering_rect(i, rect)) return false;
