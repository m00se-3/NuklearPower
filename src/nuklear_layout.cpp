...
namespace nk {
      ...
      float height, int cols, int width)
  {
    ...
    else win->layout->row.type = panel_row_layout_type::LAYOUT_STATIC_FIXED;
      ...
      float row_height, int cols)
  {
    ...
    else layout->row.type = panel_row_layout_type::LAYOUT_STATIC_ROW;
  ...
  layout_row_template_push_dynamic(struct context *ctx)
  {
    ...
    if (layout->row.type != panel_row_layout_type::LAYOUT_TEMPLATE) return;
      ...
      float height, int widget_count)
  {
    ...
    else layout->row.type = panel_row_layout_type::LAYOUT_DYNAMIC_FREE;
